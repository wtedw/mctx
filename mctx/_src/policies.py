...
def gumbel_muzero_policy_sh3(
  ...
  rng_key, key_leaf = jax.random.split(rng_key)
  leaf_keys = jax.random.split(key_leaf, Bx8x4).reshape(Bx8x4, -1)
  ...
  parent_emb_flat = jax.tree_map(gather_parents_leaf, layer1_embeds)
  leaf_parent_emb = jax.tree_map(lambda x: jnp.repeat(x, K, axis=0),
                                  parent_emb_flat)  # [B*8*K,…]

  flat2_out, _ = recurrent_fn(params, leaf_keys, leaf_actions, leaf_parent_emb)

  def unflat_leaf(x):
      return x.reshape(B, top_k_second, K, *x.shape[1:])
